...
get_lcd_fan_transform :: proc(mac_str: string, fan_index: int) -> (LCD_Transform, LCD_Config_Error) {
...
update_lcd_fan_transform :: proc(mac_str: string, fan_index: int, transform: LCD_Transform) -> LCD_Config_Error {
	config, load_err := load_lcd_config()
	defer {
		for device in config.devices {
			delete(device.mac_str)
		...
		for device, idx in config.devices {
			if idx == device_idx {
				...
				if fan_idx == -1 {
					...
					copy(new_fans, device.fans)
					new_fans[fan_idx].transform = transform
					new_device := LCD_Device_Config{
						mac_str = device.mac_str,
						fans = new_fans,
					}
